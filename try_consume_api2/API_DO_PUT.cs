...
namespace try_consume_api2
{
    class API_DO_PUT
    {
        ...
        String dev_orderid2, packing_number;
        //========================VARIABLE FOR DELIVERY ORDER HEADER======= =============================
        String date2, dev_date2, dev_orderId2, dev_time2, time2, timestamp2, store_code2, war_from2, war_to2, cust_id_store, epy_id, epy_name;
            ...
            if (ckon1.myReader.HasRows)
            {
                while (ckon1.myReader.Read())
                {
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        try
                        {
                            HttpResponseMessage message = client.PutAsync(ls.link+"/api/DeliveryOrder", httpContent).Result;
