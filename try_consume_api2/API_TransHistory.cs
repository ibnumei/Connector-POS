...
namespace try_consume_api2
{
    class API_TransHistory
    {
        ...
        public async Task getTransHistory()
        {
            String response = "";
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                try
                {
                    ...
                    HttpResponseMessage message = client.GetAsync(ls.link +"/api/GetTransactionAPI?Store="+store_code).Result;
                    //HttpResponseMessage message = client.GetAsync("http://mpos.biensicore.co.id:8082/api/GetTransactionAPI?store=ABQ").Result;
                    if (message.IsSuccessStatusCode)
                    {
                        var serializer = new DataContractJsonSerializer(typeof(List<Trans_History>));
                        var result = message.Content.ReadAsStringAsync().Result;
                        byte[] byteArray = Encoding.UTF8.GetBytes(result);
                        MemoryStream stream = new MemoryStream(byteArray);
                        List<Trans_History> resultData = serializer.ReadObject(stream) as List<Trans_History>;
