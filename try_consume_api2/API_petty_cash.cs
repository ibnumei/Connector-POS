...
namespace try_consume_api2
{
    class API_petty_cash
    {
        ...
        //=========================PETTY HEADER=======================
        String store_code, date, desc, exp_ctg_id, exp_ctg, exp_date, petty_Cash_id, time, time_stamp, cust_id_Store, seq_number_substring;
        ...
        public async Task post_pettyCAsh()
        {
            ...
            if(ckon1.myReader.HasRows)
            {
                while(ckon1.myReader.Read())
                {
                    ...
                    seq_number_substring = petty_Cash_id.Substring(12);
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        try
                        {
                            HttpResponseMessage message = client.PostAsync(ls.link+"/api/PettyCash", httpContent).Result;
                            //.HttpResponseMessage message = client.PostAsync("http://mpos.biensicore.co.id/api/PettyCash", httpContent).Result;
