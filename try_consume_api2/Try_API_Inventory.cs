            ...
            if (ckon1.myReader.HasRows)
            {
                while (ckon1.myReader.Read())
                {
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        ...
                        //HttpResponseMessage message = client.PostAsync("http://mpos.biensicore.co.id/api/ReturnOrder", httpContent).Result;
                        HttpResponseMessage message = client.PostAsync(ls.link+"/api/ReturnOrder", httpContent).Result;
